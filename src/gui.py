...
def ui_show_debug():
	st.checkbox('Display debug section', key='show_debug')
...
def ui_model():
	...
	st.selectbox('OpenAI model', models, key='model', disabled=not ss.get('api_key'))
...
def ui_question():
	st.write('### Explore your PDF document: '+(f' {ss["filename"]}' if ss.get('filename') else ''))
	disabled = False
	st.text_area('question', key='question', height=5, placeholder='Enter your question here', help='', label_visibility="collapsed", disabled=disabled)
...
def b_ask():
	...
	#if c4.button('👍', use_container_width=True, disabled=not ss.get('output')):
	...
	#c4.write(f'feedback score: {score}')
	c5.checkbox('Cache the conversation', True, key='send_details',
			help='Enable the conversation to be stored in the SagePDF feedback database')
	...
	if c1.button('Ask SagePDF', disabled=disabled, type='primary', use_container_width=False):
		...
		index = ss.get('index',{})
		with st.spinner('Hold on tight. Getting you the answer'):
...
def b_save():
	...
	if st.button('Save encrypted index of your document in SagePDF', disabled=disabled, help=help):
		with st.spinner('Saving to SagePDF'):
			db.put(name, index)
...
def b_delete():
	...
	# TODO: confirm delete
	if st.button('Delete from SagePDF', disabled=not db or not name):
		with st.spinner('Deleting from SagePDF'):
			db.delete(name)
...
st.sidebar.write('## 3. Finetune your model parameters')

with st.sidebar.expander('Parameter configuration'):
	ui_show_debug()
	b_clear()
	ui_model()
	ui_fragments()
